...
This is the canonical version. The in-app version at `/agent-guide` is rendered
from this same content.

---

## What the Creme Agent Network is

A curated directory of licensed agents who specialize in landlord-side deals:
...
## Becoming a Creme Agent

1. **Apply** at `/agent-guide` → **Register as a Creme Agent**. You'll need:
   - Real-estate license number
   - Issuing state / province
   - Headshot (square, ≥400×400)
   - Bio (≥120 chars — this is your sales copy on `/agents/:id`)
   - At least 1 specialty + 1 service area
2. **Verification.** Admin verifies your license against the state DRE / RECO
   portal. Target SLA is 24 business hours.
3. **Approval.** When approved you'll see your public profile at
   `/agents/<your-id>` and `/agents` lists you in the directory.

If you're rejected, the reason is in your dashboard. Most rejections are
fixable (expired license, blurry headshot) — fix and re-apply.

---

## Profile that gets leads

These are the patterns we see on the top 10% of profiles:

- **Bio:** 2-3 short paragraphs. Lead with what you specialize in, not where
  you grew up. Investors are scanning for "1031 exchange", "multi-unit",
  "novation", "section 8 friendly" — use the words they'll search for.
- **Specialties:** pick 3–5, not 12. The directory filters by specialty; being
  in 12 buckets means you rank in none.
- **Service areas:** list cities, not whole states. "Phoenix AZ + Mesa AZ"
  ranks better than "Arizona".
- **Headshot:** professional, square crop, neutral background. Selfies get
  passed over.
...
4. First response: phone call > SMS > email. We see a 3× higher close rate
   on phone-first responses.

## Working a lead

### Discovery (first call)
- What are they actually trying to do? (Buy / sell / refi / novate / rent up)
- Timeline?
- Have they been pre-qualified / pre-approved?
- Are they already working with another agent? (If yes, decline politely —
...
- **Response time** — median time-to-first-contact on leads. Target <1 hr.
- **Close rate** — closed transactions ÷ leads accepted. Target ≥18% for
  buy/sell, ≥35% for FSBO conversions.
- **NPS** — post-closing tenant/client survey, 0–10. Target ≥9.

Agents under target for 2 consecutive quarters move to "probation" — leads
...
- A client asks to migrate from another platform (we can help with the
  data export)
- Anything that looks like a Fair Housing violation
- A tenant complaint about a Pro member you closed with — loop in admin

Open a ticket from `/agent-guide` → **Contact admin**. Median first response is
4 business hours.
